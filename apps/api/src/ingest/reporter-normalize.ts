...
/**
 * Shared NIH RePORTER (https://api.reporter.nih.gov/v2/projects/search) normaliser.
 * Several African research programmes are NIH-funded and therefore appear in
 * RePORTER filtered by their funding-opportunity numbers — DS-I Africa and
 * H3Africa among them. The response shape is identical across programmes, so the
 * normalisation (collapse award-years per core project, derive lead org / contact
 * PI / co-PI members / NIH grant) is shared here; each adapter supplies only its
 * programme name and a fallback source URL. RePORTER carries neither ORCID nor
...
export interface ReporterOptions {
  /** Programme name written onto every project (and resolved to a programme row). */
  programName: string;
  /** Source URL used when a result lacks an appl_id to build a project-details link. */
...
/**
 * Normalise a RePORTER response into one project per core award (collapsing the
 * per-fiscal-year rows, keeping the latest year), with its lead organisation,
 * contact PI, and co-PIs as members, under the supplied programme name.
 */
export function parseReporter(jsonText: string, opts: ReporterOptions): ProjectUpsert[] {
  ...
  for (const r of latest.values()) {
    ...
    const sourceUrl = r.appl_id
      ...
      : opts.fallbackSourceUrl;
    const leadOrg = org(orgName, r.organization?.org_country ?? null, opts.fallbackSourceUrl);
